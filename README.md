...
Plugin in QGIS öffnen.

Einen DEM-Rasterlayer auswählen.

Einen Vektorlayer mit Liniengeometrie auswählen.

Eine Methode für die 3D-Berechnung auswählen:
...
Mit Aktualisieren – Refresh die Layerlisten neu einlesen, ohne das Pluginfenster neu öffnen zu müssen.

Mit Beschriftung zurücksetzen – Reset label die aktuelle Beschriftung deaktivieren.
...
Select a DEM raster layer.

Select a vector layer with line geometry.

Choose a 3D calculation method:
...
Press OK to calculate the 3D length.
...
Use Refresh to reload the available layer lists without reopening the plugin window.

Use Reset label to disable the current labeling.
